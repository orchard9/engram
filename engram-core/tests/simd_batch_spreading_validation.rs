...
fn test_batch_cosine_similarity_matches_scalar() {
    ...
    let vectors: Vec<[f32; 768]> = (0..100).map(generate_test_vector).collect();
...
fn test_batch_similarity_identical_vectors() {
    ...
    for (i, &result) in simd_results.iter().enumerate() {
        assert!(
            (result - 1.0).abs() < 1e-5,
            "Expected similarity ~1.0 for identical vectors at index {i}, got {result}"
        );
    }
}
...
fn test_sigmoid_activation_matches_scalar() {
        ...
        .enumerate()
    {
        ...
        assert!(
            ...
            "Sigmoid activation differs at index {i}: simd={simd}, scalar={scalar}"
        );
    }
}
...
fn test_sigmoid_activation_bounds() {
    ...
    let activations = SimdActivationMapper::batch_sigmoid_activation(&similarities, 0.5, 0.0);

    // All sigmoid outputs should be in [0, 1]
    for (i, &activation) in activations.iter().enumerate() {
        assert!(
            (0.0..=1.0).contains(&activation),
            "Activation out of bounds at index {i}: {activation}"
        );
    }
}
...
fn test_fma_confidence_aggregate_correctness() {
        ...
        .enumerate()
    {
        ...
        assert!(
            (simd - scalar).abs() < 1e-6,
            "FMA aggregate differs at index {i}: simd={simd}, scalar={scalar}"
        );
    }
}
...
fn test_batch_similarity_performance_characteristics() {
    ...
    for batch_size in [8, 16, 32, 64, 128] {
        let vectors: Vec<[f32; 768]> = (0..batch_size).map(generate_test_vector).collect();
        ...
        for (i, &result) in results.iter().enumerate() {
            assert!(
                ...
                "Invalid similarity at index {i} in batch {batch_size}: {result}"
            );
        }
    }
}
...
fn test_integrated_pipeline_consistency() {
    ...
    let run1 = {
        ...
        SimdActivationMapper::batch_sigmoid_activation(&similarities, 0.5, 0.1)
    };
