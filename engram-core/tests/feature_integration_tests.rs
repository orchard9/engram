...
fn test_null_completion_provider() {
    ...
    // Test completion
    let matches = completion.complete(&partial, &[candidate.clone()], 0.0).unwrap();
    ...
    // Test gap filling
    let filled = completion.fill_gaps(&partial, &[true, false, true]).unwrap();
...
fn test_feature_compatibility() {
    ...
    assert!(registry.check_compatibility(&["decay", "monitoring"]).is_ok());
    
    // All features should be compatible by default
    assert!(registry.check_compatibility(&[
        "index", "storage", "decay", "monitoring", "completion"
    ]).is_ok());
}
