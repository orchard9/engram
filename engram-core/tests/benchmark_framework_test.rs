...
fn test_statistical_power_calculation() {
    ...
    let tolerance = 10; // Allow some tolerance for rounding
...
fn test_metamorphic_relations() {
    ...
    assert_eq!(dot_product, 0.0, "Orthogonal vectors should have dot product of 0");
    ...
    assert!((self_similarity - 1.0).abs() < 1e-6, "Self similarity should be 1");
}
...
fn test_differential_testing_structure() {
    ...
    assert!(baseline_names.len() >= 5, "Should have at least 5 baseline implementations");
    assert!(baseline_names.contains(&"Pinecone"), "Should include Pinecone baseline");
    assert!(baseline_names.contains(&"Neo4j"), "Should include Neo4j baseline");
}
...
fn test_performance_cliff_detection() {
    ...
    assert!(is_cliff, "2.5x slowdown should be detected as a performance cliff");
}