...
impl EngramOptimizedAnnIndex {
    ...
    fn build_hnsw_graph(&mut self) {
        ...
        self.graph = vec![Vec::with_capacity(self.m); self.vectors.len()];
        ...
        for i in 0..self.vectors.len() {
            ...
            self.graph[i] = neighbors.into_iter().map(|(idx, _)| idx).collect();
        }
    }

    fn search_layer(&self, query: &[f32; 768], entry_points: &[usize], ef: usize) -> Vec<(usize, f32)> {
        ...
        w.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        w.into_iter().map(|(idx, sim)| (idx, sim)).collect()
    }
}
...
impl AnnIndex for EngramOptimizedAnnIndex {
    ...
    fn memory_usage(&self) -> usize {
        ...
        // Graph memory
        let graph_memory = self.graph.iter()
