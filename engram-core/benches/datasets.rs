...
use crate::ann_comparison::AnnDataset;
...
impl DatasetLoader {
    ...
    pub fn generate_synthetic(num_vectors: usize, num_queries: usize) -> Result<AnnDataset> {
        ...
        Ok(AnnDataset {
            name: format!("Synthetic_{}_{}", num_vectors, num_queries),
            vectors,
            queries,
            ground_truth,
        })
    ...
    pub fn load_sift1m_mock() -> Result<AnnDataset> {
        ...
        let vectors: Vec<[f32; 768]> = (0..num_vectors)
            .map(|_| {
                ...
                for i in 0..128 {
                    vec[i] = rng.gen_range(0.0..255.0) / 255.0;
                }
                vec
            })
            .collect();
    ...
    pub fn load_glove_mock() -> Result<AnnDataset> {
...
mod tests {
    ...
    fn test_sift_mock() {
        let dataset = DatasetLoader::load_sift1m_mock()
            .expect("Failed to load SIFT mock");
