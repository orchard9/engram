...
impl MockAnnoyIndex {
    pub const fn new(dimension: usize, n_trees: usize) -> Self {
        assert!(dimension == 768, "Only 768-dim vectors supported");
...
impl AnnIndex for MockAnnoyIndex {
    ...
    fn search(&self, query: &[f32; 768], k: usize) -> Vec<(usize, f32)> {
        ...
        if self.n_trees < 100 {
            ...
            for (_, sim) in &mut distances {
                ...
                let tree_factor = (self.n_trees as f32).max(1.0);
                let noise = (rand::random::<f32>() - 0.5) * 0.01 * (100.0 / tree_factor);
                *sim += noise;
    ...
    fn memory_usage(&self) -> usize {
        ...
        let vector_memory = self.vectors.len() * self.dimension * std::mem::size_of::<f32>();
...
mod tests {
    ...
    #[test]
    fn test_mock_annoy() {
        let mut index = MockAnnoyIndex::new(768, 10);

        let vectors = vec![[0.1f32; 768], [0.2f32; 768], [0.3f32; 768], [0.4f32; 768]];
