...
impl MockFaissIndex {
    pub fn new_ivf_flat(dimension: usize, nlist: usize) -> Self {
        assert_eq!(dimension, 768, "Only 768-dim vectors supported");

        Self {
            vectors: Vec::new(),
            index_type: format!("IVF{nlist},Flat"),
            m: 16,
        }
    }

    pub fn new_hnsw(dimension: usize, m: usize) -> Self {
        assert_eq!(dimension, 768, "Only 768-dim vectors supported");
...
mod tests {
    ...
    fn test_mock_faiss_hnsw() {
        let mut index = MockFaissIndex::new_hnsw(768, 16);

        let vectors = vec![[0.1f32; 768], [0.2f32; 768], [0.3f32; 768]];
    ...
    fn test_mock_faiss_ivf() {
        let mut index = MockFaissIndex::new_ivf_flat(768, 100);
