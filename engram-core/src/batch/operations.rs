...
use crate::batch::{
    BatchConfig, BatchOperations, BatchRecallResult, BatchSimilarityResult, BatchStoreResult,
};
...
impl BatchOperations for MemoryStore {
    fn batch_store(&self, episodes: Vec<Episode>, _config: BatchConfig) -> BatchStoreResult {
        ...
        let _initial_pressure = self.pressure();
...
impl MemoryStore {
    ...
    fn batch_recall_embeddings_simd(
        ...
        query_embeddings
            ...
            .map(|(query_embedding, cue)| {
                ...
                let similarities =
                    compute::cosine_similarity_batch_768(&query_embedding, &episode_embeddings);
    ...
    fn similarity_search_single(
        ...
        let embeddings: Vec<[f32; 768]> = episodes.iter().map(|(_, ep)| ep.embedding).collect();
