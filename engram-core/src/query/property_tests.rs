...
use super::{ConfidenceInterval, ProbabilisticQueryResult, evidence::EvidenceAggregator};
...
fn confidence_interval_strategy() -> impl Strategy<Value = ConfidenceInterval> {
    (0.0f32..=1.0f32, 0.0f32..=0.2f32).prop_map(|(center, half_width)| {
        ConfidenceInterval::from_confidence_with_uncertainty(Confidence::exact(center), half_width)
    })
}
...
proptest! {
    ...
    fn test_interval_and_monotonicity(
        ...
        // AND result should be no greater than minimum of input upper bounds
        // (since AND uses multiplication: P(A ∧ B) = P(A) * P(B) ≤ min(P(A), P(B)))
        prop_assert!(result.upper.raw() <= a.upper.raw().min(b.upper.raw()) + 1e-5);
    }
    ...
    fn test_interval_or_monotonicity(
        ...
        // OR result should be no less than max of input lower bounds
        // (since OR: P(A ∨ B) = P(A) + P(B) - P(A ∧ B) ≥ max(P(A), P(B)))
        let min_expected = a.lower.raw().max(b.lower.raw());
    ...
    fn test_activation_path_degradation(
        ...
        if path_length > 0 {
            let expected_max = activation.value() / f32::from(path_length).mul_add(0.1, 1.0);
...
mod integration_tests {
    ...
    fn test_single_high_confidence_result() {
        let episodes = vec![(create_test_episode("single"), Confidence::HIGH)];
...
mod benchmarks {
    ...
    fn bench_confidence_interval_operations() {
        let interval1 = ConfidenceInterval::from_confidence_with_uncertainty(Confidence::HIGH, 0.1);
        let interval2 =
            ConfidenceInterval::from_confidence_with_uncertainty(Confidence::MEDIUM, 0.15);
