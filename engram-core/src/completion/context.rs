...
impl GridModule {
    ...
    pub fn activation(&self, position: (f32, f32)) -> f32 {
        ...
        let rotated_x = position.0.mul_add(
            self.orientation.cos(),
            -(position.1 * self.orientation.sin()),
        );
        let rotated_y = position
            .0
            .mul_add(self.orientation.sin(), position.1 * self.orientation.cos());
        ...
        let wave1 = ((2.0
            * std::f32::consts::PI
            * shifted_x.mul_add(angle1.cos(), shifted_y * angle1.sin()))
            / self.scale)
            .cos();
        let wave2 = ((2.0
            * std::f32::consts::PI
            * shifted_x.mul_add(angle2.cos(), shifted_y * angle2.sin()))
            / self.scale)
            .cos();
        let wave3 = ((2.0
            * std::f32::consts::PI
            * shifted_x.mul_add(angle3.cos(), shifted_y * angle3.sin()))
            / self.scale)
            .cos();

        // Combine waves and normalize
        let activation = (wave1 + wave2 + wave3) / 3.0;
        f32::midpoint(activation, 1.0) // Normalize to [0, 1]
    }
}
...
impl EntorhinalContext {
    ...
    #[must_use]
    pub fn gather_temporal_context(
        &self,
        ...
        context_episodes.sort_by_key(|ep| (target_time - ep.when).num_seconds().abs());
    ...
    pub fn find_similar_grid_patterns(
        ...
        for episode in episodes {
            ...
            let similarity =
                self.grid_code_similarity(target_code, &episode.embedding[..target_code.len()]);
            if similarity > 0.5 {
    ...
    fn grid_code_similarity(&self, code1: &[f32], code2: &[f32]) -> f32 {
        ...
        let dot: f32 = code1.iter().zip(code2.iter()).map(|(a, b)| a * b).sum();
    ...
    pub fn integrate_contexts(
        ...
        let mut result: Vec<(String, Episode, f32)> = integrated
            .into_iter()
