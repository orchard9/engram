...
impl MetacognitiveConfidence {
    ...
    fn isotonic_regression_calibration(&self, raw_confidence: f32) -> f32 {
        ...
        let mut relevant_points: Vec<&CalibrationPoint> = self
            .calibration_data
            .iter()
        ...
        for window in relevant_points.windows(2) {
                ...
                && raw_confidence <= window[1].predicted_confidence
            {
                ...
                calibrated = window[0]
                    .actual_accuracy
                    .mul_add(1.0 - t, window[1].actual_accuracy * t);
                break;
            }
        }
    ...
    fn reduce_overconfidence(&self, confidence: f32) -> f32 {
        ...
        if confidence > 0.8 {
            (confidence - 0.8).mul_add(0.5, 0.8) // Halve the excess above 0.8
        } else if confidence > 0.6 {
            (confidence - 0.6).mul_add(0.75, 0.6) // Reduce moderately high confidence
        } else {
    ...
    pub fn detect_source_confusion(&self, episode: &CompletedEpisode) -> bool {
        ...
        let sources: Vec<_> = episode.source_attribution.field_sources.values().collect();
