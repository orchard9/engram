...
use super::{
    ...
    PartialEpisode, PatternCompleter, SourceMap, MemorySource, ActivationTrace,
    ActivationPathway, hippocampal::HippocampalCompletion
};
...
impl PatternReconstructor {
    ...
    fn reconstruct_embedding(&self, partial: &PartialEpisode, similar_episodes: &[&Episode]) -> [f32; 768] {
    ...
    fn calculate_confidence(&self, partial: &PartialEpisode, reconstructed_fields: usize) -> Confidence {
        let known_ratio = partial.known_fields.len() as f32 / 4.0; // 4 main fields
        let reconstructed_ratio = reconstructed_fields as f32 / 4.0;
        let base_confidence = (known_ratio + reconstructed_ratio * 0.5).min(1.0);
    ...
    fn complete(&self, partial: &PartialEpisode) -> CompletionResult<CompletedEpisode> {
        ...
        if hippocampal_result.is_err() {
            ...
            let episode = Episode {
                ...
                what: all_fields.get("what").cloned()
            ...
            for (field, _) in &reconstructed_fields {
                source_map.field_sources.insert(field.clone(), MemorySource::Reconstructed);
                source_map.source_confidence.insert(field.clone(), confidence);
            }
