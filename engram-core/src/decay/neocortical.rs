...
impl Default for NeocorticalDecayFunction {
    fn default() -> Self {
        Self {
            beta: 0.5, // Empirical value from Wixted & Ebbesen
...
impl NeocorticalDecayFunction {
    ...
    pub fn set_schema_strength(&mut self, strength: f32) {
        self.schema_strength = strength.clamp(0.1, 3.0);
    }
    
    /// Sets individual neocortical efficiency factor
    pub fn set_individual_efficiency(&mut self, efficiency: f32) {
    ...
    pub fn compute_retention(&self, elapsed_time: Duration) -> f32 {
        ...
        let effective_alpha = self.alpha * self.individual_efficiency * self.consolidation_multiplier;
        
        // Power law decay with schema-modulated parameters
        let schema_beta = self.beta / (1.0 + self.schema_strength * 0.2);
        
        #[cfg(feature = "psychological_decay")]
        let base_retention = effective_alpha * (pow((1.0 + days) as f64, -schema_beta as f64) as f32);
        ...
        if days > self.permastore_onset_days && base_retention > self.permastore_threshold {
            ...
            let stable_level = self.permastore_threshold + 
                (base_retention - self.permastore_threshold) * (1.0 - permastore_factor * 0.2);
    ...
    pub fn consolidation_boost(&self, overlap_with_schemas: f32) -> f32 {
        ...
        let boost = 1.0 + (clamped_overlap * self.schema_strength * 0.5);
    ...
    pub fn compute_half_life_days(&self) -> f32 {
        ...
        let schema_beta = self.beta / (1.0 + self.schema_strength * 0.2);
        let _effective_alpha = self.alpha * self.individual_efficiency * self.consolidation_multiplier;
        
        // Simplified half-life calculation
        #[cfg(feature = "psychological_decay")]
        { pow(2.0, (1.0 / schema_beta) as f64) as f32 - 1.0 }
        #[cfg(not(feature = "psychological_decay"))]
    ...
    pub fn will_reach_permastore(&self, current_retention: f32) -> bool {
        ...
        current_retention > self.permastore_threshold * 1.2 && 
        self.schema_strength > 0.7
    }
    
    /// Gets permastore threshold value
    pub fn permastore_threshold(&self) -> f32 {
        self.permastore_threshold
    }
    
    /// Gets time to permastore onset in days
    pub fn permastore_onset_days(&self) -> f32 {
...
mod tests {
    ...
    fn test_power_law_decay_shape() {
        ...
        // Should approach but not go below permastore threshold
        let retention_long = decay.compute_retention(Duration::from_secs(10 * 365 * 86400)); // 10 years
        assert!(retention_long >= decay.permastore_threshold());
    }
    ...
    fn test_permastore_effect() {
        ...
        // Permastore should stabilize retention
        assert!(retention_5y >= decay.permastore_threshold());
        assert!(retention_10y >= decay.permastore_threshold());
