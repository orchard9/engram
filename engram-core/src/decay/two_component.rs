...
impl TwoComponentModel {
    ...
    pub fn optimal_interval(&self) -> Duration {
        ...
        let ln_ratio = target_retention.ln() / self.retrievability.ln();
    ...
    pub fn predict_retention(&self, elapsed_time: Duration) -> f32 {
        ...
        #[cfg(feature = "psychological_decay")]
        let retention = pow(self.retrievability as f64, (days / self.stability) as f64) as f32;
...
pub fn map_to_confidence(model: &TwoComponentModel, elapsed_time: Duration) -> Confidence {
    ...
    // Map retention probability to confidence with stability weighting
    let stability_factor = (model.stability() / 10.0).min(2.0); // Stability boosts confidence
    let confidence_raw = retention * stability_factor;
    
    Confidence::exact(confidence_raw.clamp(0.01, 0.95))
}
...
mod tests {
    ...
    fn test_retention_prediction() {
        ...
        assert_eq!(immediate, 0.9); // Initial retrievability
    ...
    fn test_forgetting_decay_application() {
        ...
        model.apply_forgetting_decay(Duration::from_secs(3 * 86400)); // 3 days
        
        // Retrievability should decrease
    ...
    fn test_confidence_mapping() {
        let model = TwoComponentModel::with_parameters(0.8, 10.0, 1.0, 2.0);
        let elapsed_time = Duration::from_secs(86400); // 1 day
