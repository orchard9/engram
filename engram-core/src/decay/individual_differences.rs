...
impl IndividualDifferenceProfile {
    ...
    pub fn modify_hippocampal_tau(&self, base_tau: f32) -> f32 {
        let combined_factor = 
            self.wm_capacity * 0.25 +           // WM capacity affects encoding
            self.processing_speed * 0.25 +      // Speed affects retrieval efficiency  
            self.attention_control * 0.20 +     // Attention affects focus during encoding
            self.hippocampal_efficiency * 0.30; // Direct hippocampal efficiency
    ...
    pub fn modify_neocortical_tau(&self, base_tau: f32) -> f32 {
        let combined_factor =
            self.flexibility * 0.30 +              // Schema integration depends on flexibility
            self.neocortical_efficiency * 0.25 +   // Direct neocortical efficiency
            self.attention_control * 0.20 +        // Sustained attention for consolidation
            self.sleep_quality * 0.25;             // Sleep-dependent consolidation
    ...
    pub fn schema_integration_efficiency(&self) -> f32 {
        let efficiency = 
            self.flexibility * 0.40 +              // Primary factor for schema updating
            self.wm_capacity * 0.20 +              // WM needed to maintain schema and new info
            self.processing_speed * 0.10 +         // Speed helps with rapid integration
            self.neocortical_efficiency * 0.30;    // Neocortical function for schemas
    ...
    pub fn pattern_completion_efficiency(&self) -> f32 {
        let efficiency =
            self.wm_capacity * 0.35 +              // WM needed to maintain partial patterns
            self.hippocampal_efficiency * 0.40 +   // Direct hippocampal pattern completion
            self.attention_control * 0.25;         // Focus needed for cue processing
    ...
    pub fn retrieval_speed_factor(&self) -> f32 {
        let speed_factor =
            self.processing_speed * 0.50 +         // Primary factor for retrieval speed
            self.attention_control * 0.25 +        // Focused attention speeds retrieval
            self.hippocampal_efficiency * 0.25;    // Hippocampal efficiency for access
    ...
    pub fn consolidation_effectiveness(&self) -> f32 {
        let effectiveness =
            self.sleep_quality * 0.45 +            // Sleep is primary consolidation time
            self.neocortical_efficiency * 0.35 +   // Neocortical consolidation processes
            self.flexibility * 0.20;               // Flexibility helps memory reorganization
    ...
    pub fn interference_resistance(&self) -> f32 {
        let resistance =
            self.attention_control * 0.60 +        // Primary factor for interference control
            self.wm_capacity * 0.25 +              // WM helps maintain focus
            self.processing_speed * 0.15;          // Speed helps overcome interference
    ...
    pub fn overall_memory_efficiency(&self) -> f32 {
        let efficiency =
            self.wm_capacity * 0.20 +
            self.processing_speed * 0.20 +
            self.attention_control * 0.20 +
            self.flexibility * 0.15 +
            self.hippocampal_efficiency * 0.125 +
            self.neocortical_efficiency * 0.125 +
            self.sleep_quality * 0.10;
    ...
    pub fn calibrate_confidence(&self, base_confidence: Confidence) -> Confidence {
        ...
        let calibrated = if raw > 0.7 {
            // High confidence gets moderated by cognitive abilities
            raw * (0.85 + 0.15 * calibration_factor)
        } else {
            // Low confidence may get slight boost from good cognitive abilities
            raw * (0.95 + 0.05 * calibration_factor)
        };
    ...
    pub fn with_decline(decline_factor: f32) -> Self {
        ...
        Self {
            wm_capacity: (1.0 - clamped_decline * 0.6).clamp(0.5, 2.0),
            processing_speed: (1.0 - clamped_decline * 0.7).clamp(0.5, 2.0),
            attention_control: (1.0 - clamped_decline * 0.5).clamp(0.5, 2.0),
            flexibility: (1.0 - clamped_decline * 0.4).clamp(0.5, 2.0),
            hippocampal_efficiency: (1.0 - clamped_decline * 0.8).clamp(0.5, 2.0),
            neocortical_efficiency: (1.0 - clamped_decline * 0.3).clamp(0.5, 2.0),
            sleep_quality: (1.0 - clamped_decline * 0.5).clamp(0.5, 2.0),
    ...
    pub fn with_enhancement(enhancement_factor: f32) -> Self {
        ...
        Self {
            wm_capacity: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
            processing_speed: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
            attention_control: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
            flexibility: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
            hippocampal_efficiency: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
            neocortical_efficiency: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
            sleep_quality: (1.0 + clamped_enhancement * 1.0).clamp(0.5, 2.0),
...
mod tests {
    ...
    fn test_population_sampling() {
        ...
        for _ in 0..100 {
            profiles.push(IndividualDifferenceProfile::sample_from_population(&mut rng));
        }
        
        // Check that we get variation (not all identical)
        let first_wm = profiles[0].wm_capacity;
        let has_variation = profiles.iter().any(|p| (p.wm_capacity - first_wm).abs() > 0.1);
    ...
    fn test_consolidation_effectiveness() {
        let good_sleeper = IndividualDifferenceProfile::default();
        good_sleeper.sleep_quality = 1.6;
        
        let poor_sleeper = IndividualDifferenceProfile::default();
