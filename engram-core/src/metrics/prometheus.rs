...
pub fn export_all(registry: &MetricsRegistry) -> String {
    ...
    writeln!(&mut output, r#"engram_info{{version="0.1.0",arch="{}"}} 1"#, 
...
fn export_counters(output: &mut String, registry: &MetricsRegistry) {
    ...
    for (name, help) in counters {
        ...
        if value > 0 {
            ...
            writeln!(output, "# TYPE engram_{} counter", name).unwrap();
...
fn export_histograms(output: &mut String, registry: &MetricsRegistry) {
    ...
    let histograms = [
        ...
        ("consolidation_duration_seconds", "Consolidation duration in seconds"),
        ("activation_spreading_depth", "Activation spreading depth"),
        ("pattern_completion_iterations", "Pattern completion iterations"),
    ];
    ...
    for (name, help) in histograms {
        ...
        if quantiles.iter().any(|&v| v > 0.0) {
            writeln!(output, "# HELP engram_{} {}", name, help).unwrap();
            writeln!(output, "# TYPE engram_{} summary", name).unwrap();
            
            writeln!(output, r#"engram_{}{{quantile="0.5"}} {}"#, name, quantiles[0]).unwrap();
            writeln!(output, r#"engram_{}{{quantile="0.9"}} {}"#, name, quantiles[1]).unwrap();
            writeln!(output, r#"engram_{}{{quantile="0.99"}} {}"#, name, quantiles[2]).unwrap();
            writeln!(output).unwrap();
        }
    }
}
...
fn export_cognitive(output: &mut String, registry: &MetricsRegistry) {
    ...
    writeln!(output, "# HELP engram_cls_hippocampal_weight Hippocampal contribution weight").unwrap();
    writeln!(output, "# TYPE engram_cls_hippocampal_weight gauge").unwrap();
    writeln!(output, "engram_cls_hippocampal_weight {}", hippo).unwrap();
    writeln!(output).unwrap();
    
    writeln!(output, "# HELP engram_cls_neocortical_weight Neocortical contribution weight").unwrap();
    ...
    writeln!(output, "# HELP engram_pattern_completion_plausibility Pattern completion plausibility score").unwrap();
    writeln!(output, "# TYPE engram_pattern_completion_plausibility gauge").unwrap();
    writeln!(output, "engram_pattern_completion_plausibility {}", pc_stats.plausibility).unwrap();
    writeln!(output).unwrap();
    
    writeln!(output, "# HELP engram_false_memory_rate Rate of false memory generation").unwrap();
    writeln!(output, "# TYPE engram_false_memory_rate gauge").unwrap();
    writeln!(output, "engram_false_memory_rate {}", pc_stats.false_memory_rate).unwrap();
    writeln!(output).unwrap();
    
    // Activation depth
    let avg_depth = registry.cognitive.get_average_activation_depth();
    writeln!(output, "# HELP engram_activation_depth_average Average spreading activation depth").unwrap();
    ...
    writeln!(output, "# HELP engram_overconfidence_corrections_total Total overconfidence corrections").unwrap();
    writeln!(output, "# TYPE engram_overconfidence_corrections_total counter").unwrap();
    writeln!(output, "engram_overconfidence_corrections_total {}", cal_stats.overconfidence_corrections).unwrap();
    writeln!(output).unwrap();
}
...
fn export_hardware(output: &mut String, registry: &MetricsRegistry) {
    ...
    writeln!(output, "# HELP engram_simd_utilization_percent SIMD instruction utilization percentage").unwrap();
    ...
    for level in [CacheLevel::L1, CacheLevel::L2, CacheLevel::L3] {
        ...
        writeln!(output, "# HELP engram_cache_{}_hit_ratio Cache {} hit ratio", level_str, level_str.to_uppercase()).unwrap();
        writeln!(output, "# TYPE engram_cache_{}_hit_ratio gauge", level_str).unwrap();
        writeln!(output, "engram_cache_{}_hit_ratio {}", level_str, hit_ratio).unwrap();
        writeln!(output).unwrap();
    }
    
    // Branch prediction accuracy
    let branch_accuracy = registry.hardware.branch_prediction_accuracy();
    writeln!(output, "# HELP engram_branch_prediction_accuracy Branch prediction accuracy").unwrap();
    writeln!(output, "# TYPE engram_branch_prediction_accuracy gauge").unwrap();
    writeln!(output, "engram_branch_prediction_accuracy {}", branch_accuracy).unwrap();
    writeln!(output).unwrap();
}
...
fn export_health(output: &mut String, registry: &MetricsRegistry) {
    ...
    writeln!(output, "# HELP engram_health_status System health status (1=healthy, 0.5=degraded, 0=unhealthy)").unwrap();
    ...
    for check in report.checks {
        ...
        writeln!(output, r#"engram_health_check{{name="{}"}} {}"#, check.name, check_value).unwrap();
    }
    writeln!(output).unwrap();
}
