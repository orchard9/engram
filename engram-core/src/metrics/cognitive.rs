...
impl CognitiveMetrics {
    ...
    pub fn record(&self, metric: CognitiveMetric) {
        match metric {
            CognitiveMetric::CLSContribution { hippocampal, neocortical } => {
                self.cls_hippocampal_weight.store(hippocampal, Ordering::Relaxed);
                self.cls_neocortical_weight.store(neocortical, Ordering::Relaxed);
            }
            CognitiveMetric::PatternCompletion { plausibility, is_false_memory } => {
                self.pattern_completion_plausibility.store(plausibility, Ordering::Relaxed);
                if is_false_memory {
                    let current = self.false_memory_generation_rate.load(Ordering::Acquire);
                    // Exponential moving average
                    let new_rate = current * 0.95 + 0.05;
                    self.false_memory_generation_rate.store(new_rate, Ordering::Release);
                }
            }
            CognitiveMetric::ConsolidationTransition { from, to } => {
                self.consolidation_transitions.fetch_add(1, Ordering::Relaxed);
                self.current_consolidation_state.store(to as u64, Ordering::Release);
            }
            CognitiveMetric::SpreadingActivation { depth, branching_factor } => {
                self.activation_depth_sum.fetch_add(depth as u64, Ordering::Relaxed);
                self.activation_depth_count.fetch_add(1, Ordering::Relaxed);
                self.activation_branching_factor.store(branching_factor, Ordering::Relaxed);
            }
            CognitiveMetric::ConfidenceCalibration { correction_type } => {
                match correction_type {
                    CalibrationCorrection::Overconfidence => {
                        self.overconfidence_corrections.fetch_add(1, Ordering::Relaxed);
                    }
