...
pub struct AccessTracker {
    ...
    ewma_alpha: AtomicU32, // Stored as fixed-point value (alpha * 1000)
...
impl AccessTracker {
    ...
    pub fn new() -> Self {
        Self {
            ...
            ewma_alpha: AtomicU32::new(300), // 0.3 as default (300/1000)
    ...
    pub fn set_ewma_alpha(&self, alpha: f32) {
        self.ewma_alpha
            .store(encode_alpha(alpha), Ordering::Relaxed);
    }

    /// Get current EWMA alpha
    fn get_ewma_alpha(&self) -> f32 {
        #[allow(clippy::cast_precision_loss)]
        let alpha_raw = self.ewma_alpha.load(Ordering::Relaxed) as f32;
        alpha_raw / 1000.0
    }
    ...
    pub fn record_access(&self, memory_id: &str, activation: f32) {
        ...
        let time_delta = self.access_times.get(memory_id).map_or_else(
            || Duration::from_secs(3600),
            |entry| now.duration_since(*entry.value()).unwrap_or_default(),
        );
        ...
        self.ewma_frequencies
            .entry(memory_id.to_string())
            .and_modify(|freq| {
                *freq = alpha.mul_add(instant_frequency, (1.0 - alpha) * (*freq));
            })
    ...
    pub fn get_access_frequency(&self, memory_id: &str) -> f32 {
        ...
        self.ewma_frequencies.get(memory_id).map_or_else(
            || {
                ...
                if idle_hours < 0.001 {
                    ...
                    u64_to_f32(count) / idle_hours.max(1.0)
                }
            },
            |entry| *entry.value(),
        )
    ...
    pub fn get_average_activation(&self, memory_id: &str) -> f32 {
        self.activation_history.get(memory_id).map_or(0.0, |entry| {
            let history = entry.value();
            if history.is_empty() {
                0.0
            } else {
                let total: f32 = history.iter().sum();
                usize_to_f32(history.len()).map_or(0.0, |len| total / len.max(f32::EPSILON))
            }
        })
    }
        ...
        self.activation_history.get(memory_id).map_or(0.0, |entry| {
            ...
            if history.len() < 2 {
                ...
                if mid == 0 {
                    ...
                    let first_total: f32 = history[..mid].iter().sum();
                    let second_total: f32 = history[mid..].iter().sum();
                    let first_avg =
                        usize_to_f32(mid).map_or(0.0, |len| first_total / len.max(f32::EPSILON));
                    let second_len = history.len() - mid;
                    let second_avg = usize_to_f32(second_len)
                        .map_or(0.0, |len| second_total / len.max(f32::EPSILON));
    ...
    pub fn global_stats(&self) -> GlobalAccessStats {
        GlobalAccessStats {
            ...
            average_access_frequency: if self.access_counts.is_empty() {
                ...
                let total: u64 = self.access_counts.iter().map(|e| *e.value()).sum();
                let count = usize_to_f32(self.access_counts.len()).unwrap_or(1.0);
                u64_to_f32(total) / count.max(f32::EPSILON)
            },
        }
    }
}
...
#[inline]
fn encode_alpha(alpha: f32) -> u32 {
    let clamped = alpha.clamp(0.1, 0.9);
    let scaled = (clamped * 1000.0).round();
    if !scaled.is_finite() {
        return 300;
    }
    let bounded = scaled.clamp(100.0, 900.0);
    #[allow(
...
fn u64_to_f32(value: u64) -> f32 {
    ...
    let truncated = u32::try_from(clamped).unwrap_or(u32::MAX);
    #[allow(clippy::cast_precision_loss)]
    {
        truncated as f32
    }
...
#[inline]
fn usize_to_f32(value: usize) -> Option<f32> {
    u32::try_from(value).ok().map(|v| {
        #[allow(clippy::cast_precision_loss)]
        {
            v as f32
        }
    ...
    pub fn predict_next_access(&self, memory_id: &str) -> Option<PredictedAccess> {
        let intervals: Vec<Duration> = {
            ...
            history
                ...
                .collect::<Vec<_>>()
                .windows(2)
                .filter_map(|window| window[1].timestamp.duration_since(window[0].timestamp).ok())
                .collect()
        };
        ...
        for interval in intervals.iter().skip(1) {
            predicted_interval = self.ewma_alpha.mul_add(
        ...
        let len_f32 = usize_to_f32(intervals.len())
            .unwrap_or(1.0)
            .max(f32::EPSILON);
        let mean_interval = intervals.iter().map(Duration::as_secs_f32).sum::<f32>() / len_f32;
        let variance = intervals
            .iter()
            .map(|duration| {
                let delta = duration.as_secs_f32() - mean_interval;
                delta * delta
            })
            .sum::<f32>()
            / len_f32;
        ...
        PredictorStats {
            ...
            avg_confidence: if self.predictions.is_empty() {
                ...
                let total: f32 = self
                    ...
                    .sum();
                let count = usize_to_f32(self.predictions.len()).unwrap_or(1.0);
                total / count.max(f32::EPSILON)
            },
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_access_predictor() -> TestResult {
        ...
        let prediction = predictor
            .predict_next_access("memory1")
            .ok_or_else(|| "expected access prediction to be available".to_string())?;
        ensure(
            prediction.confidence > 0.0,
            "prediction confidence positive",
        )?;
        ensure(
            prediction.interval.as_millis() > 50,
            "prediction interval should be near 100ms",
        )?;

        // Test prediction access
        ensure(
            !predictor.is_likely_access_soon("memory1", Duration::from_millis(1)),
            "short window should not indicate imminent access",
        )?;
